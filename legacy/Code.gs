...
// ─────────────────────────────────────────────
var SHEET_ID = "TU_SPREADSHEET_ID_AQUI"; // ← pegá el ID de tu Google Sheet
...
function doGet() {
  return HtmlService.createHtmlOutputFromFile("index")
    .setTitle("Check List GTE de Guardia")
    .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
}

// ─────────────────────────────────────────────
//  GUARDAR DATOS
//  Cada sala tiene su propia hoja (pestaña).
// ─────────────────────────────────────────────
function guardarChecklist(datos) {
  try {
    var ss = SpreadsheetApp.openById(SHEET_ID);

    // Obtener o crear la hoja de la sala
    var sheetName = datos.sala; // "PASTEKO" o "STEPAKO"
